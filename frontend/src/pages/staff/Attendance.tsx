...
import type { AttendanceRecord, AttendanceSummary, PendingAttendance } from '../../types'
...
export default function Attendance() {
  ...
  function load() {
    ...
    api.get('/attendance/staff/me', { params: { from, to } }).then((res) => setRecords(res.data)).catch(() => {})
    api.get('/attendance/staff/me/summary').then((res) => setSummary(res.data)).catch(() => {})
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Your attendance record for this month.</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
          <h2 className="text-sm font-semibold text-amber-900 mb-3">Pending Approvals from Your Students ({pending.length})</h2>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm max-w-3xl">
        ...
        <div className="grid grid-cols-7 gap-3">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            const style = record ? STATUS_STYLES[record.status] : 'text-slate-400'
            const ring = record ? APPROVAL_RING[record.approvalStatus] : ''
            return (
              <div key={day} className={`aspect-square rounded-xl text-base font-medium flex items-center justify-center ${style} ${ring}`}>
                {day}
              </div>
            )
          })}
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6">
            ...
            <form onSubmit={handleMark} className="space-y-3">
              ...
              <input placeholder="Remarks (optional)" value={form.remarks}
                ...
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
              <p className="text-xs text-slate-400">The admin team will need to approve this before it counts toward your attendance.</p>
